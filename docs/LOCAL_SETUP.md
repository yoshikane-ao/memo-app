...
- 単体テスト: コンポーネント挙動、store / コマンドのテスト、ページ初期化テスト、API エラー正規化、Vite プロキシの `/health` / `/memos` / `/tags` スモーク
- ブラウザ E2E: Playwright スモーク（メモ初回ロード → 作成 → ゴミ箱 → 復元 → リロード永続化）。テスト専用のバックエンド + サーバーを Playwright が立ち上げる
