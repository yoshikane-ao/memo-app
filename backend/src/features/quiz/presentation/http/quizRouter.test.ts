...
describe("quizRouter", () => {
  ...
  it("lists quizzes with the expected response shape", async () => {
  ...
  it("creates a quiz through the application and infrastructure layers", async () => {
    ...
    const response = await request(buildJsonTestApp(quizRouter)).post("/register").send({
      word: "beta",
      mean: "second",
      tag: ["grammar"],
      groupName: "review",
          ...
          create: [
            {
              quizTag: {
                connectOrCreate: {
                  where: { tagName: "grammar" },
                  create: { tagName: "grammar" },
                },
              },
            },
          ],
  ...
  it("returns 404 when toggling favorite for a missing quiz", async () => {
    mockedPrisma.quizs.findUnique.mockResolvedValue(null);

    const response = await request(buildJsonTestApp(quizRouter)).patch("/favorite/99");

    expect(response.status).toBe(404);
    expect(response.body).toEqual({ message: "Quiz not found." });
    expect(mockedPrisma.quizs.update).not.toHaveBeenCalled();
  });

  it("bulk updates quiz tags and groups through the application and infrastructure layers", async () => {
    ...
    const response = await request(buildJsonTestApp(quizRouter)).patch("/bulk").send({
      quizIds: [1, 2],
    ...
    expect(mockedPrisma.quizTag.createMany).toHaveBeenCalledWith({
      data: [{ tagName: "existing" }, { tagName: "grammar" }],
      skipDuplicates: true,
    });
