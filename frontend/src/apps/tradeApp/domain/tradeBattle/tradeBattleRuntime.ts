...
import { calculateTradeImpactAmounts, MIN_TRADE_ORDER_AMOUNT, resolvePriceAfterDelta } from '../../lib/tradeImpact'
import {
...
export function createTradeBattleRuntime(options: {
  ...
  const applyPlayerOrder = (
    ...
    if (player.cash < requiredCash) {
      pushLog(logs, 'system', '資金不足', `${player.name}は${stock.name}の注文に必要な現金が不足しています。`, 'warn')
      return
    }
    ...
    if (payload.tradeAction === 'buy') {
      ...
      player.cash -= requiredCash
      appendTradePosition(player, payload.stockKey, 'buy', executedUnits, executionPrice, entryHistoryPointId, requiredCash)

      pushLog(logs, 'player', '買い', `${player.name}が${stock.name}を ${formatCurrency(requiredCash)} で買い。約定 ${formatCurrency(executionPrice)} / 約${formatPositionUnits(executedUnits)}口`, 'up')
      return
    }
