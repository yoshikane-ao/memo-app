...
import { AD_CAMPAIGN_ACTION, type StockKey } from '../../api/types/game'
import {
  buildBattleActionProjection,
  createDefaultBattleActionDraft,
} from '../../lib/tradeBattle'
import { findPlayerById, initializeBattleState } from '../../lib/tradeBattleState'
import {
...
describe('tradeBattleSelectors', () => {
  ...
  it('creates immutable stock snapshots and detects projected movement', () => {
    ...
    expect(hasProjectedChartMovement({
      p1: 1_000_100,
    }, state.stocks)).toBe(true)
    expect(hasProjectedChartMovement({
      p1: 1_000_000,
    }, state.stocks)).toBe(false)
  })
})
