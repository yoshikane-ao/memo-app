...
function normalizeStats(stats: Partial<TradeProfileStats> | undefined): TradeProfileStats {
  ...
  if (typeof merged.currentAssets !== 'number') {
    merged.currentAssets =
      typeof merged.totalEarnedAmount === 'number' ? merged.totalEarnedAmount : 0;
  }
...
function detectStyle(stats: TradeProfileStats): TradeProfileStyle {
  const { buyCount, sellCount, shortSellCount, companyActionCount, waitCount, wins, losses } =
    stats;
...
export const useTradeProfileStore = defineStore('tradeProfile', () => {
  const profiles = ref<TradeProfile[]>(
    parseProfiles(typeof window !== 'undefined' ? localStorage.getItem(STORAGE_KEY) : null),
  );
  const activeProfileId = ref<TradeProfileId | null>(
    typeof window !== 'undefined' ? localStorage.getItem(ACTIVE_STORAGE_KEY) : null,
  );
  ...
  function recordBattleResult(input: RecordBattleInput): void {
    ...
    const delta =
      typeof input.earnedAmount === 'number'
        ? input.earnedAmount
    ...
    nextStats.averageFinalAssets = Math.round(
      (target.stats.averageFinalAssets * target.stats.totalBattles + input.finalAssets) /
        nextStats.totalBattles,
    );
