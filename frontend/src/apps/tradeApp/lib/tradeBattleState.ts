...
import { isBattleTurnComplete, resolveNextBattlePlayer, resolveTurnLeadPlayer } from './tradeBattle'
...
export function findStockByKey(
...
export function initializeBattleState(
  ...
  state.players.forEach((player) => {
    ...
    player.startingOwnStockPrice = findStockByKey(state, resolveOwnStockKey(player.id)).currentPrice
    player.positions = []
    player.speculation = []
    player.marketBias = 0
