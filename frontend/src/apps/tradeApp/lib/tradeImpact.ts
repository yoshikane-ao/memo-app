import type { PlayerId, StockKey, TradeAction } from '../api/types/game'
...
export function normalizeStockPriceDelta(rawDelta: number): number {
  ...
  const normalizedDelta =
    Math.max(STOCK_PRICE_TICK, Math.round(absoluteDelta / STOCK_PRICE_TICK) * STOCK_PRICE_TICK)
...
export function resolvePriceAfterDelta(
  ...
  const normalizedDelta = !Number.isFinite(rawDelta) || rawDelta === 0
    ? 0
    : Math.sign(rawDelta) * Math.max(priceStep, Math.round(Math.abs(rawDelta) / priceStep) * priceStep)
