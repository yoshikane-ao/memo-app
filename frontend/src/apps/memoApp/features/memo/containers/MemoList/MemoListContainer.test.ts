...
const MemoListStub = defineComponent({
  ...
  emits: ["reorder-requested", "save-requested", "trash-requested", "memo-tags-updated", "tag-deleted"],
  template: `
    <div>
      ...
      <button class="emit-trash" @click="$emit('trash-requested', 1)">trash</button>
      <button
        class="emit-tags"
...
describe("MemoListContainer", () => {
  ...
  it("uses confirm before moving a memo to trash and relays tag deletion upward", async () => {
    storeMock.moveMemoToTrash.mockResolvedValue({ ok: true, value: undefined });
    ...
    await wrapper.get(".emit-trash").trigger("click");
    await flushPromises();
    expect(window.confirm).toHaveBeenCalledWith("このメモをごみ箱に移動しますか？");
  ...
  it("skips reorder when disabled and shows feedback on failed save/trash", async () => {
    storeMock.updateMemo.mockResolvedValue({ ok: false, reason: "error" });
    storeMock.moveMemoToTrash.mockResolvedValue({ ok: false, reason: "error" });
