...
import type { TagSummary } from "./memo.types";
...
import {
  cloneTagSummary,
  toTagSummary,
  toUniqueTagSummaries,
  toValueCommandResult,
  toVoidCommandResult,
} from "./commandHelpers";

type TagCommandDependencies = {
...
export const createTagCommandHandlers = ({
  memoStore,
  tagStore,
  history,
  ...
  const createTag = async (input: CreateTagInput): Promise<CommandResult<TagItem>> => {
    ...
    return toValueCommandResult<TagItem>(result, finalTag);
  };
