...
const MemoCardActionsStub = defineComponent({
  ...
  emits: ["copy-requested", "save-requested", "trash-requested"],
  template: `
    <div>
      ...
      <button class="trash-btn" @click="$emit('trash-requested')">trash</button>
    </div>
  `,
});
...
describe("MemoCard", () => {
  ...
  it("syncs incoming memo props and relays trash/tag events", async () => {
    const wrapper = mount(MemoCard, {
      props: {
        memo: makeMemo(),
      },
    ...
    await wrapper.get(".trash-btn").trigger("click");
    ...
    expect(wrapper.emitted("trash-requested")?.[0]).toEqual([1]);
  });
});
