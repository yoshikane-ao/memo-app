...
describe("useMemoListView", () => {
  ...
  it("sorts trash views by deletedAt when configured", () => {
    const items = ref([
      makeMemo({ id: 1, deletedAt: "2026-03-23T10:00:00.000Z" }),
      makeMemo({ id: 2, deletedAt: "2026-03-25T10:00:00.000Z" }),
      makeMemo({ id: 3, deletedAt: "2026-03-24T10:00:00.000Z" }),
    ]);
    const view = useMemoListView(items, {
  ...
  it("treats whitespace-only keywords as inactive filters", () => {
    const items = ref([makeMemo(), makeMemo({ id: 2, title: "Beta" })]);
    const view = useMemoListView(items);

    view.keyword.value = "   ";

    expect(view.displayedMemos.value.map((memo) => memo.id)).toEqual([1, 2]);
    expect(view.canReorder.value).toBe(true);
  });
});
